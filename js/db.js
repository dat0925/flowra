...
export const DB = {
  ...
  // Premiumプランの月次AI上限
  // 2026-06-17に300回へ変更（旧6,000回）。さらに同日中に200回へ再調整。
  // 根拠: flowra-aiはclaude-haiku-4-5-20251001（入力$1/出力$5 per Mトークン、$1=¥160換算）。
  // 最も重い「free」質問タイプ（直近3ヶ月の全取引データをプロンプトに埋め込む）は
  // 標準的な利用量（180取引/3ヶ月）で入力約4,700トークン・出力約70トークン ≈ ¥0.81/回。
  // 当初これを基準にPremium原価率70%目標で300回（¥398×0.7÷¥0.81≈344回）と算出したが、
  // 同じユーザーが同月にレシートOCR（月200回上限、最悪ケース約¥0.55/回 ≈ ¥110）も
  // 同時に使い切る可能性を考慮すると、AI300回（最悪ケース約¥243）と合わせて
  // Stripe決済手数料込みの実質収益（¥398-¥14≈¥384）に対して余裕が¥31しかなく薄かった。
  // OCRの200回上限と揃える形で200回に再設定（AI最悪¥162＋OCR最悪¥110＝¥272、残り¥112）。
  PREMIUM_AI_LIMIT: 200,
  ...
  async getUserPlan() {
    const details = await this.getUserPlanDetails();
    return details.plan;
  },

  // ユーザーのプラン詳細（プラン名・有効期限・解約予約の有無）を取得
  // 設定画面のプランバッジ表示など、期限情報が必要な箇所はこちらを使う。
        ...
        if (ownerUserId) {
          ...
          if (ownerPlanData) {
            ...
            if (!ownerExpired && (ownerPlan === 'premium' || ownerPlan === 'admin')) {
              return { plan: ownerPlan, expiresAt: null, cancelAtPeriodEnd: false };
            }
          }
        }
      }
