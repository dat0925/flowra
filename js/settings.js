...
async function renderTagList(tags) {
  ...
  wrap.innerHTML = tags.map(t => {
    ...
    return '<div class="tag-item" data-tag-id="' + t.id + '" style="display:flex;align-items:center;padding:0 16px;border-bottom:1px solid var(--border);">'  
      ...
      + '<span style="width:72px;text-align:right;font-size:14px;font-weight:600;color:var(--ink);">'
        + (budgetVal || '<span style="color:var(--mid-lt);">−</span>') + '</span>'
...
function openTagEditSheet(tag, allTags, budgetMap = {}) {
  ...
  document.getElementById('btn-delete-tag')?.addEventListener('click', async () => {
    ...
    try {
      ...
      closeSheet();
      showToast('タグを削除しました');
      const updatedTags2 = await DB.getTags();
      await renderTagList(updatedTags2);
    } catch (e) {
