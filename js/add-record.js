...
export async function renderAddRecord(onSave, onReady, initialState = {}) {
  ...
  function bindEvents() {
    ...
    document.getElementById('btn-acct')?.addEventListener('click', () => {
      showAccountPicker({ accounts, currentId: state.accountId, title: '口座を選択',
        onSelect: id => { state.accountId = id; render(); } });
    });
    ...
    document.getElementById('btn-to-acct')?.addEventListener('click', () => {
      showAccountPicker({ accounts, currentId: state.toAccountId, title: '移動先の口座',
        onSelect: id => { state.toAccountId = id; render(); } });
    });
...
function autoAssignTags(itemName, tags, budgetMap) {
  ...
  const TAG_KEYWORDS = {
    ...
    // 食材カテゴリ
    '食費': [
      '食','米','パン','野菜','肉','魚','卵','乳','豆腐','納豆',
      '調味','ソース','醤油','味噌','油','塩','砂糖','小麦','片栗',
      // 麺・主食
      'スパゲ','うどん','そば','ラーメン','パスタ','そうめん','春雨','ビーフン',
      'ご飯','餅','もち','赤飯',
      // 肉・魚・加工品
      '鶏','豚','牛','羊','ラム','ベーコン','ハム','ソーセージ','ウインナー',
      'ナゲット','フライドチキン','チキン','ミンチ','ひき肉','唐揚','とり','とんかつ',
      'まぐろ','サーモン','鮭','さば','いわし','あじ','えび','かに','たこ','いか',
      'かまぼこ','ちくわ','はんぺん','さつま揚','魚肉',
      // 野菜・果物
      'きゅうり','トマト','にんじん','ほうれん','キャベツ','レタス','玉ねぎ','たまねぎ',
      'じゃがいも','さつまいも','ねぎ','ニンニク','にんにく','生姜','しょうが',
      'ブロッコリー','なす','ピーマン','パプリカ','もやし','きのこ','しいたけ',
      'バナナ','りんご','みかん','いちご','ぶどう','メロン','スイカ','桃','梨',
      // 乳製品・卵
      'バター','チーズ','牛乳','ヨーグルト','生クリーム','豆乳','アーモンドミルク',
      // 調味料・油・粉
      'マヨネーズ','ケチャップ','ドレッシング','ポン酢','だし','みりん','酢',
      'オリーブ油','ごま油','サラダ油','コショウ','七味','カレー粉','ハーブ',
      'イースト','ベーキング','コーンスターチ','でんぷん','ゼラチン',
      // 冷凍・惣菜・弁当
      '冷凍','惣菜','弁当','寿司','刺身','おにぎり','サンドイッチ','総菜',
      // 缶詰・レトルト
      '缶詰','レトルト','インスタント','即席','缶',
      // 調味料・スパイス追加
      'シナモン','ターメリック','パプリカパウダー','クミン','バジル','オレガノ',
      'ローリエ','ナツメグ','カルダモン','スパイス','ハーブ','粉','フレーク',
      'オリーブオイル','オリーブ油','ごま','胡麻','きな粉','片栗粉',
      // きのこ・乾物
      'しめじ','えのき','まいたけ','なめこ','エリンギ','きのこ','キノコ',
      'わかめ','ひじき','のり','海苔','昆布','乾物','切干','高野豆腐',
      // アイス・冷菓
      'アイス','ジェラート','シャーベット','アイスクリーム','ソフト',
      // スナック・菓子（商品名パターン）
      'ポテト','プリッツ','ビスコ','チップ','クラッカー','おかき','あられ',
      // 汁物・鍋
      '味噌汁','スープ','鍋','おでん',
    ],
    // ── 外食 ──────────────────────────────────────────────────
    '外食': [
      'レストラン','カフェ','ランチ','ディナー','テイクアウト','デリバリー',
      'マクドナルド','マック','ケンタッキー','kfc','スタバ','スターバックス',
      'すき家','吉野家','松屋','なか卯','丸亀','天丼','かつや',
      '餃子の王将','くら寿司','スシロー','はま寿司','かっぱ寿司',
      'はなまる','ガスト','サイゼ','バーミヤン','デニーズ','ジョナサン',
      'ドミノ','ピザ','モスバーガー','ロッテリア','フレッシュネス',
      '焼肉','ラーメン店','居酒屋','定食','食堂',
    ],
    // ── 飲み物 ────────────────────────────────────────────────
    '飲み物': [
      'ドリンク','飲料','コーヒー','coffee','tea','コーラ','ジュース',
      'お茶','緑茶','麦茶','烏龍茶','ほうじ茶','紅茶','水','炭酸',
      'スポーツドリンク','ポカリ','アクエリ','ゲータレード',
      'ビール','酒','ワイン','チューハイ','ウィスキー','ハイボール','日本酒','焼酎',
      'カルピス','ヤクルト','乳酸菌',
      // ティー・コーヒー商品名
      'ルイボス','ティーバッグ','茶葉','ハーブティー','カモミール','ペパーミント',
      'ほうじ','玄米茶','アールグレイ','ダージリン','アポトル','ジョージア',
      'ネスカフェ','UCC','ドトール','タリーズ','エスプレッソ','カフェラテ',
    ],
    // ── 菓子 ──────────────────────────────────────────────────
    '菓子': [
      'お菓子','チョコ','チョコレート','クッキー','ケーキ','アイス','キャンディ',
      'グミ','ポテチ','ポテトチップス','スナック','煎餅','せんべい','和菓子',
      'ショコラ','プリン','デザート','ゼリー','ガム','あめ','飴',
      'ビスケット','ウエハース','マカロン','シュークリーム','どら焼き','まんじゅう',
    ],
    // ── 日用品 ────────────────────────────────────────────────
    '日用品': [
      '洗剤','シャンプー','リンス','コンディショナー','ティッシュ','トイレ',
      'タオル','歯ブラシ','歯磨き','石鹸','ゴミ袋','ラップ','アルミ','スポンジ',
      'ハンドソープ','柔軟剤','漂白剤','洗濯','ボディソープ','ボディウォッシュ',
      'キッチンペーパー','ペーパータオル','綿棒','絆創膏','生理','ナプキン',
      'ウェットティッシュ','除菌','消臭','芳香剤','防虫',
      'トイレットペーパー','トイレロール',
    ],
    // ── 子育て ────────────────────────────────────────────────
    '子育て': [
      'おむつ','おもちゃ','玩具','絵本','ベビー','子供','キッズ','こども',
      '離乳','粉ミルク','哺乳','ベビーフード','幼児','学童','習い事',
    ],
    // ── 医療 ──────────────────────────────────────────────────
    '医療': [
      '薬','医療','病院','クリニック','処方','風邪薬','頭痛','胃腸',
      '湿布','マスク','消毒','サプリ','ビタミン','鎮痛','解熱','整腸',
      '目薬','点眼','鼻炎','花粉','アレルギー','うがい薬','塗り薬',
    ],
    // ── 美容 ──────────────────────────────────────────────────
    '美容': [
      '化粧','コスメ','美容','スキンケア','リップ','ファンデ','アイシャドウ',
      'マスカラ','シートマスク','パック','ヘアケア','美白','保湿','日焼け止め',
      '乳液','化粧水','クレンジング','洗顔','アイライナー','マニキュア',
    ],
    // ── 交通 ──────────────────────────────────────────────────
    '交通': [
      '電車','バス','タクシー','駐車','ガソリン','交通','高速','定期',
      'ic','suica','pasmo','toica','icoca','はやかけん','nimoca',
      '新幹線','特急','バス代','電車代',
    ],
    // ── 衣類 ──────────────────────────────────────────────────
    '衣類': [
      '服','シャツ','パンツ','スカート','コート','ジャケット','靴','スニーカー',
      '帽子','バッグ','下着','ソックス','靴下','ワンピース','セーター','カーディガン',
      'ジーンズ','デニム','Tシャツ','ブラウス','タイツ','レギンス',
    ],
    // ── 電化製品 ──────────────────────────────────────────────
    '電化製品': [
      '家電','スマホ','パソコン','充電','ケーブル','イヤホン','電池','電球',
      'usb','hdmi','アダプター','充電器','モバイルバッテリー',
    ],
    // ── 嗜好品 ────────────────────────────────────────────────
    ...
    const itemRows = itemStates.map((item, idx) => {
      ...
      const checkBox = item.checked
        ? '<div data-item-check="' + idx + '" style="width:44px;height:44px;flex-shrink:0;display:flex;align-items:center;justify-content:center;margin:-6px 0;cursor:pointer;">'
          + '<div style="width:22px;height:22px;border-radius:6px;background:#2d5a3d;border:2px solid #2d5a3d;display:flex;align-items:center;justify-content:center;">'
          + '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#ffffff" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>'
          + '</div></div>'
        : '<div data-item-check="' + idx + '" style="width:44px;height:44px;flex-shrink:0;display:flex;align-items:center;justify-content:center;margin:-6px 0;cursor:pointer;">'
