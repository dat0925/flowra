...
## ✨ 機能: 残高を「現在」に正し、「月末予定」を追加（2026-06-22）

**背景/課題**: `accounts.balance` はDBトリガーで全取引（未来日付ぶんも含む）が即時加算された
running total になっており、未来日の入力が即「残高」に反映されてしまっていた。本来 残高=「今この口座に
あるお金」であるべき、というプロダクト判断（他社=マネーフォワード/Zaim/カケイプラス も口座残高は支払日まで
動かさない）に合わせて、表示を是正した。

**仕様（確定事項）**:
1. 表示する「残高」は **今日(JST)時点**（未来分を除外）。全口座で表示額が変わる。
2. 各口座／総額に「**月末予定**」を出す。ただし**現在と差があるときだけ**表示（ノイズ回避）。
3. 月末予定は未来の**収入・支出・振替すべて**を符号付きで考慮（収入+/支出−/振替=出元−・移動先+）。
   総額では振替の両足が相殺されるため、収入・支出のみ効く。
4. 手動残高調整（口座編集）は「**入力＝今日時点の残高**」として扱い、保存時に未来分を内部で足し戻す。
   編集画面の「現在の残高」プリフィルも今日時点の値を表示。

**実装（DBトリガーには一切触れていない／いつでも戻せる）**:
- 新規 `js/projection.js`: `jstTodayStr` / `jstMonthEndStr`（Asia/Tokyo固定）、`computeProjections(accounts, futureTxns)`、
  `loadProjections(accounts)`、`futureSumForAccount()`。導出式は
  `現在 = balance − Σ符号付き(date>今日)`、`月末予定 = balance − Σ符号付き(date>今月末)`。
- `js/db.js`: `getTransactionsAfter(dateStr)` を追加（type/amount/account_id/to_account_id/date のみ・未来分は少数なので全件）。
- `js/accounts.js`: 一覧の残高・%バー・合計を現在残高ベースに。差があれば各口座/合計に「月末予定」行。
  編集モーダルはモジュール変数 `_proj` から現在残高/未来分を取得し、プリフィル・差分・保存の足し戻しに使用。
- `js/dashboard.js`: 描画後に `patchTotalProjection()` で総残高を現在に補正し、差があれば「月末予定」をサブ行に差し込む
  （即時描画を妨げない非同期パッチ。残高非表示中は何もしない）。
- フォールバック: 未来取引の取得に失敗したら従来どおり `balance` を表示（月末予定は出さない）。

**注意/今後**: 残高はあくまで「running total から未来分を引いた導出値」。将来トリガーを廃して
台帳から都度計算する方式（anchor列導入）に寄せると、より素直になる（今回は低リスク優先で見送り）。
当日付(date==今日)の取引は現在残高に含む。
