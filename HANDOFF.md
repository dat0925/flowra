...
1. **タグへの「固定費フラグ」追加**（中期・AIアドバイス精度向上）

   **背景:** 現在は過去3ヶ月のブレ幅で固定費を自動推定しているが、データが少ないユーザーや月によってブレる固定費（保険・習い事等）で推定が外れるケースがある。

   **設計:**
   - `tags` テーブルに `is_fixed_cost boolean DEFAULT false` カラムを追加
   - 設定画面のタグ編集シートに「固定費として扱う」トグルを追加（1行のみ）
   - `getTagBreakdown` の戻り値に `isFixed` を含める
   - Edge Functionへの `fixedCostTags[]` は「手動フラグ優先、未設定は自動推定」で合成
   - 設定任意・設定した人だけ精度が上がる設計（未設定ユーザーは現状の自動推定で動作継続）

   **必要なSQL:**
   ```sql
   ALTER TABLE tags ADD COLUMN is_fixed_cost boolean DEFAULT false;
   ```

   **触るファイル:** `settings.js`（タグ編集シート）、`db.js`（updateTag）、`dashboard.js`（getTagBreakdown / estimateFixedCostTags の合成処理）、`flowra-ai/index.ts`（変更不要）
