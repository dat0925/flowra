...
- 振替の口座を一括で直したい場合は、現状は1件ずつ編集画面から修正する必要がある
  （今後の課題: 振替の一括変更に対応する場合は「振替元を変更」「振替先を変更」を
  別ボタンに分けるなどのUI設計が必要）

**削除の一括操作について**:
- `confirm()`による1段階確認のみ（既存の税込み変換確認ダイアログと同じ簡易パターンを踏襲。
  `settings.js`のタグ削除のような2段階確認は今回は採用していない）
- 誤操作時の復旧手段は用意していない（削除は取り消し不可）ため、件数が多い場合は
  操作前に対象を落ち着いて確認するよう促す文言をconfirm内に含めている
...
**今後の検討候補**:
- 振替の一括口座変更対応
- 削除の2段階確認化（誤操作リスクを踏まえて要検討）
- カテゴリ（タグ）の一括変更（今回はスコープ外、口座変更と削除のみ実装）
