...
## ⚠️ 本番移行チェックリスト（次のTODO）

⚠️ **要確認（2026-06-18時点）**: 下記チェックリストは「LPのPayment Linkがまだサンドボックス（`test_...`）」という前提で書かれているが、実際に`lp/index.html`を確認すると現在のリンクは`https://buy.stripe.com/00w00i1Rx4Lwbmy2c3fQI02`で、`test_`プレフィックスが付いていない（＝本番リンクの形式に見える）。すでに本番化が完了していてこのチェックリストの更新が漏れているのか、まだSecrets側はサンドボックスのままなのか、Stripeダッシュボードで実際のモードを確認してから本チェックリストの扱いを判断すること。

Stripe サンドボックス → 本番切り替え手順（この順番で進める）：
...
3. **LP の Payment Link を差し替え**
   - `lp/index.html` の `buy.stripe.com/test_...` を本番 Payment Link に変更（1行）
...
5. **テストデータのクリーンアップ**
   - `user_plans` テーブルのテスト決済で作られた Stripe Customer ID を削除 or 本番 ID に差し替え
