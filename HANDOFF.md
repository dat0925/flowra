...
**追記（同日）: 検索行のUIオーバーフロー修正**
- 上記実装直後、実機（iPhone）で検索中に「✕・件数バッジ・スコープ切替・集計・選択」
  の5要素が1行に収まらずはみ出す不具合が発覚
- 3段化・横スクロールではなく、「検索中に優先度が低い要素を隠す」方針で対応
  （`setSecondaryActionsVisible()`で検索中は集計・選択ボタンをhidden化）
- 件数バッジとスコープ切替ボタンは`updateSearchScopeChip()`で1つのチップに統合
  （「50件・全期間 ▾」のようにタップで直近1ヶ月⇄全期間を切替）
- 検索開始時に選択モードが有効だった場合は`exitSelectionMode()`で自動解除
...
出ることがある。画面を下にスワイプすると直る。

**原因分析（Taskra比較で判明）**:
- Taskra（dat0925/task-app）は `.bot-nav` が `position:fixed;bottom:0` で画面底に直接ピン留め
  されており、高さ計算に依存しないためこの問題が構造的に起きない。
- Flowraはflex縦積みで、全体高さを `--app-h`（JSが `window.innerHeight` を測って設定）に依存。
  iOS PWAはキーボード閉じ直後に古い（小さい）innerHeightを返すことがあり、その値で
  `--app-h` が固定されると隙間が出る。スワイプでresizeが発火して測り直されるため直る。

**今回の対策（js/app.jsのみ、CSS無変更）**:
- `_setAppHRetry()` を新設：測定を即時＋100/300/600ms遅延で計4回行い、iOSが正しい値を
  返したタイミングを拾う。
- resize / visualViewport.resize を retry版に差し替え、`focusout`（キーボード閉じ）と
  `pageshow`（復帰時）でも再測定するよう追加。
- **例の禁止事項（`#main { height:100dvh }` を触らない等）は一切触れていない。**
  戻す場合はこのcommitをrevertするだけでよい。

**根本治療案（未実施）**: Taskra方式で `#bottom-nav` を `position:fixed` 化＋コンテンツに
padding-bottom。原因自体が消えるが、レイアウト影響が大きいため今回は見送り。
上記の遅延再測定で不十分な場合の次の一手として検討。

**⚠️ 追記（同日）: JS補正はどちらも失敗。やり直さないこと**
- 遅延リトライ（afc882c）: 効果なし。innerHeightはスワイプまで古い値を返し続けるため、
  何回測り直しても同じ古い値になる。
- visualViewport.height + offsetTop 方式（9c01d13）: **悪化**（スワイプでも直らなくなった）。
  d9ba3c2でrevert済み。scrollイベントでの再測定も逆効果だった。
- 結論: この問題はJSの測定タイミング調整では治らない。次にやるなら根本治療
  （Taskra方式のfixed化）を**別ブランチ＋プレビュー環境で検証してから**。本番mainで
  直接試行しないこと。

---

## 🎨 UI調整: 記録編集シートの削除/複製ボタンの視覚的強調（2026-07-12）

**内容**: 記録編集シート（`js/edit-record.js`）の下部ボタン2点をさりげなく調整。
- 削除ボタン: 枠線・文字色をグレー（`var(--mid)`）→ 赤（`var(--red)`）に変更し、
  「危険な操作」であることが一目でわかるようにした。背景・太さは変更なし（派手にしすぎない）。
- 複製ボタン: 枠線をsage（`var(--sage-lt)`）、背景を薄いsage塗り（`var(--sage-bg)`）、
  文字色を濃いsage（`var(--sage-dk)`）・太字に変更し、使用頻度が高いことを踏まえて
  共有ボタンより一段目立つようにした。保存ボタン（濃いグリーン塗り）ほどは主張しない中間の強さ。

**認証・決済・DBへの影響**: なし（純粋にインラインスタイルのみの変更）。RLS等のセキュリティ
チェックは対象外と判断。

**注意**: `js/accounts.js`の口座削除ボタン（`btn-delete-acct`、530行目付近）は同じ見た目の
パターンだが、今回は対象外（依頼が記録編集シートのみだったため）。統一したい場合は次の対応で。

**デプロイ**: このリポジトリは`has_pages: false`（GitHub Pages未使用）。`flowra.taskra.jp`は
Vercel経由でのデプロイと思われる。pushすれば自動デプロイされる想定だが、Vercel側のビルド状況は
GitHub APIからは確認できないので、必要なら別途Vercelダッシュボードで確認すること。

---

## 🐛 バグ修正: ホーム画面の収入・支出がプルトゥリフレッシュ後に実際と違う金額のまま表示される（2026-07-11）

**事象**: ホーム画面で下スワイプ（プルトゥリフレッシュ）すると、収入・支出の金額が一瞬記録一覧
（`records`）画面と異なる金額に変わり、しかもその「違う方の金額」がそのままホームに表示され
続けてしまう。記録一覧側の金額の方が正しい。

**いつから発生していたか**:
`e743a1d`（2026-05-25「perf: Service Worker + IndexedDB cache + optimistic UI」）で
IndexedDBキャッシュとStale-While-Revalidate方式（キャッシュ即表示→バックグラウンド差分同期）
を導入した時点から、原理的にはずっと発生し得る状態だった。ただし顕在化するには次のいずれかが
過去に一度でも行われている必要があるため、実際にユーザーが違和感として気づくまでには時間差が
あったとみられる（※日付の変更は原因ではない。同一IDのレコードのdateフィールドが書き換わる
だけで、月別表示は読み出し時にdateで絞り込む方式のため、日付変更単体ではゴーストは発生しない。
下記2点のみが実際の発生条件）：
1. 記録一覧の**一括削除**（複数選択→削除）を使った場合。単体編集画面からの削除は
   `markDeletedTransaction()`でその場で削除フラグが立つため影響なし。一括削除経路
   （`refreshAfterBulkChange()`）だけが単純な`upsertTransactions`頼みの再取得になっており、
   削除した取引がキャッシュに残っていた。
2. **この端末以外（他デバイス・別タブ等）で取引が削除された**場合。削除フラグは操作した
   端末のローカルキャッシュにしか立たないため、この端末側は月次の正解データ置き換え処理
   （今回修正した箇所）が正しく動くまで、消えたはずの取引をキャッシュに持ち続けてしまう。

**原因は2つの問題の組み合わせ**:

1. **キャッシュに「ゴースト」が残り続ける（根本原因）**
   `cache.js`の`upsertTransactions()`はIndexedDBに対して`put`しかしない設計だった。
   つまり「追加・更新」はできるが、サーバー側で削除された取引・別の月に日付が変更された
   取引をキャッシュから消す処理が一切なかった。`putAccounts()`は削除済み口座対策として
   `clear()`してから書き直す実装になっていたが、取引側には同じ対策が入っていなかった
   （非対称な実装漏れ）。この結果、削除・移動された取引がIndexedDBに「ゴースト」として
   残り続け、ローカル計算の収支合計に混入して実際より多い金額になっていた。

2. **静かな更新処理が、無関係な条件でスキップされていた（症状を悪化させていた要因）**
   `dashboard.js`の`syncInBackground()`は、プルトゥリフレッシュ時にサーバーから当月の
   正しいデータ（`result.data`）を取得し、キャッシュへの保存自体は毎回行っていた。
   しかし画面上の金額表示を更新する処理は、それとは別に`DB.getDelta(lastSync)`
   （直近同期以降に変更された行、という別の目的の差分取得）が空でない場合に限る、
   という条件が付いていた。取得した当月データに変化があっても`getDelta`側がたまたま
   空を返すケースでは、画面はSTEP1で最初に表示した（ゴースト混入済みの）古い金額の
   ままになり、裏側のキャッシュだけが更新されるという食い違いが起きていた。
   記録一覧（`records.js`）側は同じような構成ながら、常に取得した正解データ
   （`result.data`）をそのまま使って再描画・サマリー更新していたため、この問題が
   出ておらず、ホーム画面とのズレとして表面化していた。

**修正内容**:
- `js/cache.js`: `replaceMonthTransactions(year, month, rows)`を新設。指定月について
  サーバーから取得した正解データ（`rows`）に存在しないIDをキャッシュ上から物理削除して
  から書き直す（`putAccounts`と同じ「clear→書き直し」の思想を月単位に適用）。
- `js/dashboard.js`: `syncInBackground()`で当月データの保存を`upsertTransactions`から
  `replaceMonthTransactions`に変更。また、画面上の収入・支出の数字は`getDelta`の結果に
  関係なく、その場で取得済みの`result.data`（正解データ）を使って必ず更新するように変更。
- `js/records.js`: 当月データ取得箇所（`renderRecords`の STEP2、`refreshAfterBulkChange`）
  も同様に`replaceMonthTransactions`を使うよう統一し、記録一覧側でもゴーストが残らない
  ようにした。

**再発防止策**:
- **「ローカルキャッシュに全件上書き（正解データ）を書き込む処理」を新設する際は、
  必ず「サーバー側にもう存在しないIDの削除」もセットで行う**こと。`put`のみのupsertは
  「増える・更新される」ケースしか正しく扱えず、「消える」ケースを黙って見逃す設計に
  なりやすい。口座（`putAccounts`）は最初からこの対策が入っていたので、今後キャッシュに
  新しいエンティティ種別を追加する場合はそちらを参考にすること。
- **「画面に表示中の数字を更新するかどうか」の条件分岐に、表示する数字の計算に使わない
  別目的のフラグ・差分（今回で言う`getDelta`）を条件として混ぜない**こと。「取得済みの
  正解データがあるなら、それをそのまま表示に反映する」を基本とし、パフォーマンス上の
  最適化（差分のみ更新等）はあくまで「その正解データの反映」を妨げない形で追加する。
- 同種のキャッシュ不整合バグは体感しにくく（金額が「なんとなく多い」程度にしか見えない
  ため）気づかれにくい。ホーム画面・記録一覧のように同じ集計を複数画面で表示している
  箇所は、定期的に画面間の金額突き合わせを目視確認するか、可能であれば自動テストで
  `getCachedTransactions`の合計と`DB.getMonthlySummary`（サーバー側集計）の合計が
  一致することを検証する仕組みを検討する余地がある。

**セキュリティチェック**: 認証・決済・新規テーブル・個人情報カラムの変更なし
（既存の`transactions`ローカルキャッシュ〈IndexedDB〉の整合性ロジックのみを修正）。
Supabase側のRLSやテーブル定義には一切触れていない。

**触れていない箇所**: `js/db.js`（サーバー側クエリ）・DBスキーマ・認証フロー・
Stripe決済まわりは今回一切変更していない。

---


## ✨ 機能追加: 一括編集に「カテゴリ変更」「削除の2段階確認」を追加（2026-07-09）

前回（同日）実装した一括編集機能への追記。以下3点をユーザーと相談の上、方針を確定した。

**① 振替の一括口座変更は対応不要と判断（見送りではなく非対応で確定）**
- 前回は「今後の課題」としていたが、対応不要と判断したため`js/records.js`のロジックは変更していない
- 引き続き、振替（transfer）は口座の一括変更対象外のまま。振替の口座を直したい場合は
  1件ずつ編集画面から修正する（この制約は今後も維持する設計判断として確定）

**② 削除を2段階確認に変更**
- 従来の`confirm()`による1段階確認をやめ、`settings.js`のタグ削除と同じ「1回目でボタンを赤く
  ハイライト→3秒以内に2回目タップで実行、3秒経過で自動リセット」のパターンに統一
- `js/records.js`にモジュールレベルの`deleteConfirmPending`/`deleteConfirmTimer`を追加。
  選択内容が変わったとき（チェックのON/OFF・全選択・選択モード終了）は`resetDeleteConfirm()`で
  必ずリセットする（別の選択に対して「もう一度タップ」状態が残らないようにするため）

**③ カテゴリ（タグ）の一括変更を実装**
- 一括操作バーに「カテゴリを変更」ボタンを追加（口座を変更ボタンの隣・2列レイアウト）
- タップすると`js/records.js`内の`showTagPicker()`（新規・ボトムシート）でタグ一覧を表示。
  `account-picker.js`と同じ構造を踏襲しつつ、記録一覧専用のためモジュール分離はせず
  `records.js`内に閉じて実装（他画面から使い回す予定が今のところないため）
- 選択したタグ1件で、対象記録の既存タグを**全て置き換える**（主タグ・サブタグの区別なく、
  選んだタグ1件のみが付く状態になる）。複数タグを組み合わせて付けたい場合は今回は非対応
  （記録編集画面で個別に調整する運用を想定）
- `js/db.js`に`bulkUpdateTransactionTags(ids, tagId)`を追加。`transaction_tags`を対象IDで
  一旦全削除してから新タグで一括insertする実装。RLSの「静かな失敗」対策として、insert後の
  件数がids件数と一致するか検証する既存パターンを踏襲

**セキュリティチェック**: 前回同様、新規テーブル・新規列・認証・決済の変更なし。
`transaction_tags`は既存テーブル・既存RLSに対する操作のため対象外と判断。

**今後の検討候補（更新）**:
- 複数タグ（主タグ＋サブタグ）を一括で付け直したい場合の対応（現状は1タグへの置き換えのみ）
- ~~振替の一括口座変更対応~~ → 対応不要と判断（確定・再検討しない）

---

## ✨ 機能追加: 記録一覧に一括編集（口座変更・削除）を追加（2026-07-09）

**経緯**: レシートOCRで20件以上を一括登録した際、口座選択を誤ると従来は「全件やり直し」に
なってしまうというユーザー報告があった。開発者自身も同じミスを踏んだことから、単発の
バリデーション強化（デフォルト空欄化など）ではなく、間違えても直しやすい設計を優先した。

**設計判断（優先順位）**:
1. 一括登録後に事後訂正できる仕組み（コスト削減。これが一番効くという判断）
2. 「登録直後専用のUndo」ではなく、いつでも使える汎用の一括編集として実装
   （後日通帳と照合して気づくケースや、手動登録分の訂正にも同じ機能を使い回せるため）
3. デフォルト空欄化などの入力時バリデーションは今回は見送り（別途検討の余地あり）

**実装内容**:
- `js/records.js`:
  - フィルタバーに「選択」トグルボタンを追加。ONにすると各記録の左側にチェックボックスが表示され、
    タップで編集画面を開く代わりに選択ON/OFFになる（`selectionMode` / `selectedIds`で状態管理）
  - 画面下部に固定の一括操作バー（`#bulk-action-bar`）を動的生成。「キャンセル」「N件選択中」「全選択」
    と、「口座を変更」「カテゴリを変更」「削除」のボタンを表示（削除の2段階確認・カテゴリ変更は
    後日の追記セクション参照）
  - 「口座を変更」: 既存の共通モジュール`account-picker.js`（`showAccountPicker`）を再利用してピッカー表示
  - 月切り替え・検索実行時は選択状態を破棄（`exitSelectionMode()`を呼ぶ）
  - 一括操作後は`refreshAfterBulkChange()`で当月キャッシュ or 検索結果を再取得して再描画
- `js/db.js`:
  - `bulkUpdateTransactionAccount(ids, accountId)` / `bulkDeleteTransactions(ids)` を追加
  - RLSの「静かな失敗」対策として、`.select('id')`で返ってきた件数がリクエスト件数と一致するかを
    必ず検証してから成功扱いにする（既存の`getBudgets`等と同じ防御パターンを踏襲）

**⚠️ 振替（transfer）は口座の一括変更対象外**:
- 振替は`account_id`（振替元）と`to_account_id`（振替先）の2つの口座を持つため、
  「一括でどちらを変更するか」が一意に決まらない
- 選択中に振替が含まれる場合、口座変更ピッカーのタイトルに「振替N件は対象外」と表示した上で、
  収入・支出（income/expense）のみを変更対象とする（`onBulkChangeAccount()`内でtypeフィルタ）
- 振替の口座を一括で直したい場合は、現状は1件ずつ編集画面から修正する必要がある。
  **この制約への対応は不要と判断済み（上記追記セクション参照・再検討しない）**

**セキュリティチェック（実施済み・対象外と判断した理由）**:
- 新規テーブル・新規列の追加なし、認証・決済フローの変更なし
- 既存の`transactions`テーブル（RLS有効・`updateTransaction`/`deleteTransaction`と同じ権限モデル）
  に対する一括版の操作を追加しただけのため、今回のセキュリティ必須チェックリストの対象外と判断

**触れていない箇所**: DBスキーマ・認証フロー・Stripe決済まわりは今回一切変更していない
